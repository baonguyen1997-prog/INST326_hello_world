...
import sys
import argparse
class City:# This class holds data representing a City. We assume that Cities have names and neighbors.
        #assume that the neighbor relationships which have associated values, for example the distance between the cities and the interstates

        #initialize the neighbor following the attributes cities and interstates.
    def __init__(self, name:str):
        self.name = name #name of the city
        neighbors ={}  #empty dictionary
    ...
    def add_neighbor(self, neighbor(City), distance:str, interstate:str):  
            #mark values into its data types it should be in. Initialize the conditioner 'if' for neighbor names. 
            #So if the name is not belongging to the neighbor name, it would be displayed as distance and interstate. Otherwise, it will display only distance.

            if(self.name != neighbor.name):
                neighbor = {neighbor:{(distance,interstate)}}
...
class Map: 
    ...
    #Map  class  stores  the  map  data  in  the  form  of  Graph 


    def __init__ (self, relationships):

        cities =[] #cities is in an empty list
    ...
    def bsf(Graph:map,start:str, Goal:str):
        explored = []
    ...
    def main(start:str, destination:str, Graph):
